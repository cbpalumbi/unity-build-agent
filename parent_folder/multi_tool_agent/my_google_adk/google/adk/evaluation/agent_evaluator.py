...
class AgentEvaluator:
  ...
  def find_config_for_test_file(test_file: str):
    ...
    #print("hello looking for custom criteria")
    
    test_folder = os.path.dirname(test_file)
    #print("hello testfolder ", test_folder)
    config_path = os.path.join(test_folder, "test_config.json")
    #print("hello testfolder ", config_path)
    if os.path.exists(config_path):
      #print("hello looking for custom criteria 2")
  ...
  async def evaluate_eval_set(
    ...
    for eval_case_responses in eval_case_responses_list:
      ...
      for metric_name, threshold in criteria.items():
        ...
        print(f"\n--- DEBUGGING TOOL TRAJECTORY ---")
        print(f"Expected Invocations (from JSON): {expected_invocations}")
        print(f"Actual Invocations (from Agent): {actual_invocations}")
        print(f"Evaluation Result Score: {evaluation_result.overall_score}")
        print(f"Evaluation Status: {evaluation_result.overall_eval_status}")
        print(f"Metric Name: {metric_name}")
        print(f"Threshold: {threshold}")
        print(f"--- END DEBUGGING TOOL TRAJECTORY ---\n")
